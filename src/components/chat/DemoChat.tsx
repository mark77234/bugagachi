...
import { ArrowRight, MapPin, Send, UserRound, X } from "lucide-react";
...
/** 입력창 바로 위에 뜨는 플로팅 힌트. 누르면 그대로 질문이 전송된다.
 *  첫 질문을 어떻게 시작할지 막막한 사용자에게 서비스 사용법을 물어보도록 유도한다. */
const FLOATING_HINTS = ["어떻게 사용하는 거예요?", "무엇을 도와줄 수 있어요?"];
...
export function DemoChat() {
  ...
  const sendQuestion = async (question: string) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: historyForApi, housingId , location }),
      });
      if (!res.ok || !res.body) {
  ...
  // 아직 아무것도 물어보지 않았을 때만 힌트를 띄운다.
  const showHints = !hintsDismissed && !messages.some((m) => m.role === "user");
    ...
    <div className="mx-auto max-w-4xl overflow-hidden rounded-[var(--radius-cardlg)] border border-border bg-surface shadow-[var(--shadow-card)]">
      ...
      <div className="flex items-center gap-3 border-b border-border bg-gradient-to-r from-primary-subtle to-surface p-4 sm:p-5">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-lg font-extrabold tracking-tight text-navy">AI 갈붕</h2>
      ...
      <div className="grid h-[calc(100dvh-30rem)] min-h-[300px] grid-rows-[minmax(0,1fr)_auto] sm:h-[min(700px,calc(100dvh-16rem))] sm:min-h-[520px]">
        ...
        <div className="relative">
          {/* 입력창 바로 위 플로팅 힌트 — 사용법을 물어보도록 유도한다. */}
          {showHints && (
            <div className="pointer-events-none absolute inset-x-0 bottom-full z-10 px-3 pb-2 sm:px-4">
              <motion.div
                initial={reduceMotion ? false : { opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}
                className="pointer-events-auto flex items-center gap-2 overflow-x-auto rounded-full border border-primary/20 bg-surface/95 py-1.5 pl-2 pr-1.5 shadow-[var(--shadow-card)] backdrop-blur"
              >
                <Mascot pose="pointUp" className="h-8 w-8 shrink-0" sizes="32px" />
                {FLOATING_HINTS.map((hint) => (
                  <button
                    key={hint}
                    type="button"
                    disabled={streaming}
                    onClick={() => sendQuestion(hint)}
                    className="shrink-0 whitespace-nowrap rounded-full bg-primary-subtle px-3.5 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary hover:text-white focus-visible:outline focus-visible:outline-3 focus-visible:outline-[var(--color-ring)] disabled:opacity-50"
                  >
                    {hint}
                  </button>
                ))}
                <button
                  ...
                  aria-label="힌트 닫기"
                  className="ml-auto flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-muted transition-colors hover:bg-surface-muted hover:text-fg focus-visible:outline focus-visible:outline-3 focus-visible:outline-[var(--color-ring)]"
                >
